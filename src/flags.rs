...
/// F6 — Ghost racing in The Climb. Default ON per Decision D1. When OFF, the
/// ghost code paths short-circuit and the feature has ZERO behavioral effect.
...
mod tests {
    ...
    #[test]
    fn ghost_racing_default_on_and_overridable() {
        set_test_override(None);
        assert!(ghost_racing(), "D1: ghost_racing defaults ON");
