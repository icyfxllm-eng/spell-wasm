...
/// CC-WORDGRID v1 Phase B — Spell Cross's runtime kill-switch. Default ON;
/// which languages play is the launch set in `wordsearch::lexicon`. App only.
#[cfg(not(feature = "web"))]
pub fn spell_cross() -> bool {
    resolve(stored("spell_cross").as_deref(), true)
}
