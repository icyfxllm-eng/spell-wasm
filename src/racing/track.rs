...
mod tests {
    ...
    fn gated_language_with_no_production_bank_offers_no_circuits() {
        // Every registered language ships a real bank since the 2026-07-25
        // ar/hi promotions, so every one offers circuits — the empty-bank branch
        // (generate -> None, never a padded or silently-English track) is pinned
        // by the pure generate() contract tests above instead of a live language.
        for (code, _, _, _) in crate::consts::BUILTIN_LANGS.iter() {
            assert!(!available(code, "easy").is_empty(), "{code} offers no circuits");
        }
    }
