...
mod tests {
    ...
    fn registry_parses_and_holds_the_shipped_modes() {
        let all = all();
        assert_eq!(all.len(), 15, "15 modes registered");
        ...
        assert_eq!(
            ids(&all),
            vec!["practice", "ghost_racing", "syllable_replay", "say_it", "photo_list", "spell_aloud", "word_stories", "online_spelloff", "def_match", "letter_forge", "word_chains", "word_picture", "reports", "calendar", "translate"],
        );
    }
    ...
    fn little_speller_sees_only_kidsafe_modes() {
        ...
        // CC-HUB-CLEANUP D5 menu, reconciled twice since: Word Picture per
        // its own note, and the CC-REPORTS Quest Log — the kid tab IS a kid
