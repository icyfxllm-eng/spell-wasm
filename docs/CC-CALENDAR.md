...
3. Forward Planner
Drag/add words onto future days from four sources: Ready-for-a-rematch (FSRS due forecast), Words to Conquer, Trap Boss word sets, and My Words (existing custom-word path — already profanity-screened at import; the planner adds no new free-text entry). Planned words for a day become guaranteed session inclusions, band-legality filtered — an illegal-for-band word is declined at planning time with one honest audited reason, never silently dropped at session time. Future days also preview the scheduler's own forecast ("8 reviews coming due"), visually distinct from kid-planned words.
