...
| D2 | Priors and hazard taxonomies | English taxonomy + default BKT priors drafted first as the template (extends CC-PICTURE-BANK D2). Every non-English taxonomy, confusion matrix, placement set and letter-name grammar requires native-speaker audit before its language's release flag opens. **DECIDED (Eric, 2026-07-31)**: English taxonomy is the template and ships first. Tagalog → Paul. Every other language's release flag stays shut until a named native speaker signs off; owners are recorded here as they are found, and CI blocks an unaudited language's flag in the meantime. Not naming them now is the decision, not a deferral of it. |
...
| D5 | Privacy | **DECIDED**: all learner state on-device only; zero telemetry; export/delete via the existing progress-reset flow; guardian report shares only by explicit user action; speech audio never stored. Any A/B retention study (eval #1b) runs on-device with Eric-reviewed, opt-in, aggregate-only reporting — or is a TestFlight survey instead. **No silent experimentation on players.** *Annotated 2026-09-18 (CC-TELEMETRY-FOUNDATION R9, Eric): "zero telemetry" means zero learning or gameplay data. Crash and performance diagnostics under CC-TELEMETRY-FOUNDATION v1.1 are permitted: no word, answer, attempt, miss, hint, outcome or mode lifecycle ever leaves the device.* |
| D6 | Placement length | **DECIDED (Eric, 2026-07-31)**: 10–14 words per language, ≤3 minutes, skippable. |
