...
| **G-C · Pace bands (D6)** | ⛔ OPEN (Phase 4 landed with PROPOSED values) | Bronze–Platinum ship with first-cut anchors in `config/pace-bands.json`; Champion is `gated: true` (excluded). All five bands' timing params are proposed, NOT calibrated against tester data. | Approve/adjust the five bands' `base_ms/per_char_ms/jitter_pct` + the calibration method; then un-gate Champion. Bands are already live behind the dev door for on-device feel-testing. |
...
### Phase 4 — Pace ghosts (F3)  ✅ **LANDED** *(Champion gated on G-C)*
**Goal:** deterministic synthetic opponents, 5 bands.
- `src/racing/pace.rs` + `config/pace-bands.json` (one entry per band, NOT per
  language). `generate(band, words, …, seed)` builds a `RaceGhost` whose per-lap time
  is `base_ms + per_char_ms·chars` ± `jitter_pct`%, the jitter a hash of
  `(band.id, seed, lap)` — so **same band + same track seed ⇒ byte-identical** ghost
  (acceptance #2, tested). 6 tests.
- **Bronze–Platinum ship** (`active_bands()`); **Champion is `gated: true`** — excluded
  from the shipped opponent list until D6 sign-off. Its params in `pace-bands.json`
  are **PROPOSED, not approved** (see G-C below).
- **Screen integration:** `screen::opponents_for` now lists the active pace bands over
  a representative track (stable `PACE_SEED`) **before** the garage ghosts, so a fresh
  garage always has something to race. i18n: `racing.pace.{bronze,silver,gold,platinum}`
  + `racing.pace.tag`, 11 locales (324 keys, parity green).
- **DEV DOOR (temporary — remove at activation):** five quick taps on the SPELL logo
  (`brandMark`) open the still-hidden screen so Eric can test on-device. Invisible to
  normal testers (no tile, no hub entry); ghost_racing untouched. `screen::register_brand_tap`.
- **Covers acceptance:** #2. Full suite 255/0 (was 249/0).
- **⛔ Before ship (G-C/D6):** the five bands' `base_ms/per_char_ms/jitter_pct` are a
  first proposal, NOT calibrated against tester data. Bronze ≈ 4.8s/5-char word …
  Platinum ≈ 2.4s; Champion ≈ 1.75s (gated). Eric to approve/adjust + sign off the
  calibration method before these leave the dev door for real users.
