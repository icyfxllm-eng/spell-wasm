...
| **G-B · Word-ID foundation** | 📐 **DESIGN READY** | The ghost format references words by `(languageCode, wordListHash, wordID)`, never by string. Today word banks are plain `&[&str]` — **no IDs, no hash**. Every determinism/round-trip test depends on this layer. It is the real first project. | **Design proposed:** `docs/spell-racing-wordid-design.md` (content-derived FNV-1a-64 IDs, per-tier list hash, purely additive; validated collision-free on 14,368 real words). Approve its 4 open decisions, then Phase 0 code lands. |
