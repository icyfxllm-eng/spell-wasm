...
fn flow(img: &Image<f32, 1>, window: usize) -> Vec<f32> {
    let size = img.size();
        ...
        let px: Vec<f32> = (0..w * h)
            .map(|i| {
                ...
                match k % 3 {
                    ...
                    1 => (((x - 128.0).powi(2) + (y - 128.0).powi(2)).sqrt() * 0.1).sin() * 0.5 + 0.5,
...
fn main() {
    ...
    for (name, w, h, px) in &corpus {
        ...
        let _ = writeln!(report, "{{\"img\":\"{name}\",\"bands\":\"{bh:016x}\",\"flow\":\"{fh:016x}\"}}");
    }
    print!("{report}");
    println!("{{\"corpus\":\"{corpus_hash:016x}\",\"arch\":\"{}\",\"n\":{}}}", std::env::consts::ARCH, corpus.len());
}
