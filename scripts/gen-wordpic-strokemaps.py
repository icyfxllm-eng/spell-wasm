...
def stack(x, y, size, units, order, band=2):
    """A vertical letter stack at (x,y), `units` letters tall (the budget)."""
    return {"mode": "stack", "x": x, "y": y, "size": size,
            "order": order, "budget": [max(2, units - 2), units + 2],
            "band": band, "arch": "stack"}
...
P.append(pic("smiley", "easy", "faces", "🙂", True, [
    ...
    flow(arc(160, 320, 256, 420, 352, 320), 5, [4, 10], 1, "arc", 3),     # THE MOUTH — finale, 3 words
]))
...
P.append(pic("fish", "easy", "animals", "🐟", True, [
    ...
    flow(arc(210, 250, 250, 300, 300, 262), 5, [3, 8], 2, "arc"),          # fin
...
P.append(pic("house", "easy", "objects", "🏠", True, [
    ...
    flow(line(120, 400, 392, 400), 5, [4, 10], 1, "line", 2),
    flow(arc(226, 400, 256, 330, 286, 400), 6, [3, 8], 3, "arc"),          # door — finale
]))
...
P.append(pic("cat", "medium", "animals", "🐱", True, [
    ...
    flow(line(170, 190, 196, 120), 3, [3, 7], 2, "line"),                  # ear L up
    flow(line(196, 120, 226, 180), 4, [3, 7], 2, "line"),                  # ear L down
    flow(line(286, 180, 316, 120), 5, [3, 7], 2, "line"),                  # ear R up
    flow(line(316, 120, 342, 190), 6, [3, 7], 2, "line"),                  # ear R down
    flow(arc(160, 360, 130, 430, 220, 470), 7, B_MED, 2, "arc"),           # body L
    flow(arc(352, 360, 382, 430, 292, 470), 8, B_MED, 2, "arc"),           # body R
    flow(line(220, 470, 292, 470), 9, [3, 8], 2, "line"),                  # base
    flow(arc(352, 430, 452, 420, 442, 330), 10, B_MED, 2, "wave"),         # tail
    flow(line(130, 262, 196, 268), 11, [3, 6], 3, "line"),                 # whisker L
    flow(line(316, 268, 382, 262), 12, [3, 6], 3, "line"),                 # whisker R
    stack(226, 232, 22, 3, 13),                                            # eye L
    stack(296, 232, 22, 3, 14),                                            # eye R
    flow(arc(226, 292, 256, 322, 286, 292), 15, [4, 9], 3, "arc"),         # smile — finale
]))
...
P.append(pic("rocket", "medium", "vehicles", "🚀", True, [
    ...
    flow(line(226, 340, 170, 420), 4, [3, 7], 2, "line"),
    flow(line(170, 420, 226, 400), 5, [3, 7], 2, "line"),
    flow(line(292, 340, 342, 420), 6, [3, 7], 2, "line"),
    flow(line(342, 420, 292, 400), 7, [3, 7], 2, "line"),
    flow(arc(226, 262, 256, 232, 292, 262), 8, [3, 7], 3, "arc"),          # window top
    flow(arc(226, 262, 256, 292, 292, 262), 9, [3, 7], 3, "arc"),          # window bottom
    flow(line(120, 60, 180, 110), 10, [3, 6], 3, "line"),                  # star streak 1
    flow(line(392, 90, 442, 130), 11, [3, 6], 3, "line"),                  # star streak 2
    flow(arc(226, 420, 256, 500, 292, 420), 12, [4, 9], 2, "arc"),         # flame outer — finale-ish
...
P.append(pic("snowman", "medium", "holidays", "⛄", True, [
    ...
    flow(line(196, 150, 316, 150), 5, [3, 8], 2, "line"),                  # hat brim
    stack(238, 90, 22, 3, 6),                                              # hat side L
    stack(278, 90, 22, 3, 7),                                              # hat side R
    flow(line(170, 330, 96, 280), 8, [3, 7], 2, "line"),                   # arm L
    flow(line(342, 330, 416, 280), 9, [3, 7], 2, "line"),                  # arm R
    stack(256, 320, 22, 4, 10),                                            # buttons
    stack(232, 196, 18, 2, 11),                                            # eye L
    stack(282, 196, 18, 2, 12),                                            # eye R
    flow(arc(230, 246, 256, 268, 282, 246), 13, [4, 9], 3, "arc"),         # smile — finale
]))
...
_dr.append(flow(arc(170, 400, 130, 390, 120, 350), 6, [3, 7], 1, "arc"))
_dr.append(flow(arc(120, 350, 60, 330, 50, 270), 7, B_HARD, 2, "wave"))        # tail 1
_dr.append(flow(arc(50, 270, 45, 220, 80, 210), 8, [3, 7], 2, "wave"))         # tail 2
_dr.append(flow(line(60, 235, 30, 210), 9, [3, 6], 2, "line"))                 # spade a
_dr.append(flow(line(30, 210, 62, 196), 10, [3, 6], 2, "line"))                # spade b
...
for k in range(4):                                                              # back spikes
    x0 = 210 + k * 40
    _dr.append(flow(line(x0, 212 - (k % 2) * 6, x0 + 18, 184), 17 + k, [2, 6], 3, "zigzag"))
_dr.append(stack(408, 282, 20, 3, 21, 3))                                      # eye
_dr.append(flow(line(452, 320, 500, 300), 22, [3, 7], 3, "line"))               # fire 1
_dr.append(flow(line(452, 334, 504, 334), 23, [3, 7], 3, "line"))               # fire 2
_dr.append(flow(line(452, 348, 500, 366), 24, [3, 7], 3, "line"))               # fire 3
_dr.append(flow(arc(180, 430, 256, 460, 330, 430), 25, B_HARD, 3, "arc"))       # ground shadow
_dr.append(flow(arc(360, 300, 396, 316, 372, 336), 26, [4, 8], 4, "arc"))       # nostril-smile — finale
...
_ei.append(flow(line(204, 292, 232, 200), 5, [3, 8], 2, "line"))
_ei.append(flow(line(308, 292, 280, 200), 6, [3, 8], 2, "line"))
for k in range(6):                                                              # lattice X mid
    y = 280 - k * 13
    if k % 2 == 0:
        _ei.append(flow(line(212 + k * 2, y, 300 - k * 2, y - 26), 7 + k, [2, 6], 3, "zigzag"))
    else:
        _ei.append(flow(line(300 - k * 2, y, 212 + k * 2, y - 26), 7 + k, [2, 6], 3, "zigzag"))
_ei.append(flow(line(222, 200, 290, 200), 13, [3, 8], 2, "line"))               # platform 2
_ei.append(flow(line(236, 200, 248, 120), 14, [3, 7], 2, "line"))
_ei.append(flow(line(276, 200, 264, 120), 15, [3, 7], 2, "line"))
for k in range(4):                                                              # upper lattice
    y = 190 - k * 17
    if k % 2 == 0:
        _ei.append(flow(line(240 + k, y, 272 - k, y - 17), 16 + k, [2, 5], 3, "zigzag"))
    else:
        _ei.append(flow(line(272 - k, y, 240 + k, y - 17), 16 + k, [2, 5], 3, "zigzag"))
_ei.append(flow(line(246, 120, 266, 120), 20, [2, 6], 2, "line"))
_ei.append(stack(256, 68, 20, 3, 21, 2))                                        # spire
for k in range(3):                                                               # ground
    _ei.append(flow(line(80 + k * 20, 430 - k * 4, 200 + k * 30, 430 - k * 4), 22 + k, [3, 8], 3, "line"))
_ei.append(flow(arc(60, 120, 110, 80, 160, 120), 25, [4, 9], 3, "arc"))          # cloud L
_ei.append(flow(arc(360, 90, 410, 55, 460, 90), 26, [4, 9], 3, "arc"))           # cloud R
_ei.append(flow(line(410, 400, 470, 400), 27, [3, 7], 3, "line"))
_ei.append(flow(line(50, 400, 110, 400), 28, [3, 7], 3, "line"))
_ei.append(flow(arc(230, 110, 256, 92, 282, 110), 29, [3, 7], 3, "arc"))
...
_ml.append(flow(arc(206, 130, 198, 200, 216, 268), nx(), [3, 8], 3, "outline"))  # face L
_ml.append(flow(arc(306, 130, 314, 200, 296, 268), nx(), [3, 8], 3, "outline"))  # face R
_ml.append(flow(arc(216, 268, 256, 306, 296, 268), nx(), [3, 8], 3, "arc"))      # jaw
_ml.append(flow(line(238, 320, 274, 320), nx(), [3, 8], 3, "line"))              # neck base
_ml.append(flow(line(232, 296, 232, 336), nx(), [2, 6], 3, "line"))              # neck L
_ml.append(flow(line(280, 296, 280, 336), nx(), [2, 6], 3, "line"))              # neck R
_ml.append(flow(arc(222, 176, 238, 168, 252, 176), nx(), [3, 6], 3, "arc"))      # brow L
_ml.append(flow(arc(260, 176, 274, 168, 290, 176), nx(), [3, 6], 3, "arc"))      # brow R
_ml.append(stack(238, 192, 12, 2, nx(), 3))                                      # eye L
_ml.append(stack(274, 192, 12, 2, nx(), 3))                                      # eye R
_ml.append(flow(line(256, 200, 256, 236), nx(), [2, 5], 3, "line"))              # nose
_ml.append(flow(arc(244, 224, 256, 230, 268, 224), nx(), [2, 5], 3, "arc"))      # nostril line
...
for k in range(5):                                                                # drapery falls R
    x = 300 + k * 14
    _ml.append(flow(arc(x, 340, x + 6, 400, x - 4, 468), nx(), B_EXP, 2, "wave"))
_ml.append(flow(arc(230, 330, 256, 344, 282, 330), nx(), [3, 8], 2, "arc"))      # neckline
_ml.append(flow(arc(210, 350, 256, 372, 302, 350), nx(), [3, 9], 2, "arc"))      # bodice
...
for k in range(4):                                                                # fingers
    _ml.append(flow(line(296 + k * 12, 470 + (k % 2) * 4, 330 + k * 10, 486), nx(), [2, 5], 3, "line"))
_ml.append(flow(arc(236, 246, 256, 252, 276, 246), nx(), [3, 7], 4, "arc"))      # upper lip shadow
_ml.append(flow(arc(234, 256, 256, 270, 278, 256), nx(), [4, 9], 4, "arc"))      # THE SMILE — final word
